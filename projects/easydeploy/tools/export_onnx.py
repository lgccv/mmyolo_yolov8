...
def parse_args():
    ...
    parser.add_argument('--fp16-export',action='store_true',help='dynamic export model')
...
def main():
    ...
    if args.fp16_export:
        deploy_model.half()
    dtype = torch.float16 if args.fp16_export else torch.float32

    fake_input = torch.randn(args.batch_size, 3,
                             *args.img_size,dtype = dtype).to(args.device)
    # dry run
    deploy_model(fake_input)

    if args.fp16_export:
        save_onnx_path = os.path.join(args.work_dir,os.path.basename(args.checkpoint).replace('.pth', '_fp16.onnx'))
    else:
        save_onnx_path = os.path.join(args.work_dir,os.path.basename(args.checkpoint).replace('pth', 'onnx'))
